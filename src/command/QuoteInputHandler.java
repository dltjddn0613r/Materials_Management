...
import service.QuoteInputRequest;
import service.QuoteInputService;

public class QuoteInputHandler implements CommandHandler { // 폼을 보여주고 폼 전송을 처리

    private static final String FORM_VIEW = "/WEB-INF/view/quoteInput.jsp"; // 견적 입력 폼을 표시하기 위한 JSP 뷰의 경로
    private QuoteInputService quoteInputService = new QuoteInputService(); // 견적 입력 로직을 처리하는 서비스 인스턴스
    ...
    private String processSubmit(HttpServletRequest req, HttpServletResponse res) { // POST 요청 시 폼 데이터를 처리하는 메서드
        ...
        QuoteInputRequest quoteInputReq = createQuoteInputRequest(req); // 요청 파라미터를 기반으로 QuoteInputRequest 객체 생성
		/*
		 * quoteInputReq.validate(errors); // quoteInputReq 객체가 유효한지 검사
		 */
        if (!errors.isEmpty()) { // 만약 오류가 존재한다면, 폼을 다시 보여줍니다.
            return FORM_VIEW;
        }

        int newQuoteNo = quoteInputService.quoteInput(quoteInputReq); // 견적 입력 서비스 호출하여 데이터를 저장하고 새로운 견적 번호를 반환받음

        req.setAttribute("newQuoteNo", newQuoteNo); // 새로운 견적 번호를 request 객체에 추가하여 뷰에 전달

        return "/WEB-INF/view/newQuoteSuccess.jsp"; // 성공적으로 견적이 입력되었을 때 응답 페이지
    }

    private QuoteInputRequest createQuoteInputRequest(HttpServletRequest req) { // 요청에서 파라미터를 읽어 QuoteInputRequest 객체를 생성하는 메서드
        ...
        return new QuoteInputRequest( // 요청에서 파라미터를 가져와서 QuoteInputRequest 객체 생성
